...
function mw_popup() {
    global $g4, $mw, $member, $_COOKIE, $gr_id, $mg_id, $mm_id, $ms_id, $is_admin;

    $gr_id = $mg_id;

    $ret = "";
    //$ret.= "<script src=\"$g4[path]/js/jquery.js\"></script>\n";
    $ret.= "<script src=\"$g4[path]/js/jquery.easydrag.js\"></script>\n";
    $ret.= "<script>\n";
	...
	if ($_COOKIE[$popup_name] == "no") continue; // 쿠키
        if ($row['pp_url']) {
            $url = parse_url($row['pp_url']);
            if ($url['path'] != $_SERVER['PHP_SELF']) continue;
        }
        elseif ($row[gr_id] == "index" && defined("_MW_INDEX_") && !$gr_id) ; // 인덱스 팝업
        else if ($row[gr_id] == "") ; // 모든 팝업
        else if ($row[gr_id] == $gr_id) { // 그룹 팝업
            if (!$row['mm_id']) ;
            else if ($row['mm_id'] == $mm_id) {
                if (!$row['ms_id']) ;
                else if ($row['ms_id'] == $ms_id) ;
                else continue;
            }
            else continue;
        }
