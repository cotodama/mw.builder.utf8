...
?>
    ...
    <button class="fa-button" onclick="win_open('<?=$social_commerce_path?>/order_list.php?bo_table=<?=$bo_table?>', 'order_list', 'width=800,height=600,scrollbars=1');"><i class="fa fa-shopping-cart"></i> 주문내역</button>
    <? } ?>

    <? if ($mw_basic[cf_talent_market] && $is_admin) { ?>
    <button class="fa-button" onclick="win_open('<?=$talent_market_path?>/order_list.php?bo_table=<?=$bo_table?>', 'order_list', 'width=800,height=600,scrollbars=1');"><i class="fa fa-shopping-cart"></i> 주문내역</button>
    <? } ?>

    <? if ($is_admin && $mw_basic[cf_collect] == 'rss' && file_exists("$g4[path]/plugin/rss-collect/_lib.php")) {?>
    <button class="fa-button" onclick="win_open('<?=$g4[path]?>/plugin/rss-collect/config.php?bo_table=<?=$bo_table?>', 'rss_collect', 'width=800,height=600,scrollbars=1')"><i class="fa fa-wifi"></i> RSS수집</button>
    <? } ?>
...
다운:<?=$board[bo_download_point]?>"><!--
    ...
    <? if ($is_admin == "super") { ?><a href="<?=$config_href?>" class="fa-button"><i class="fa fa-gear"></i> 배추스킨설정</a><?}?>
